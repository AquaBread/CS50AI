...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    def pass_prob(parent_gene_count):
        """
        Probability a parent passes the gene variant to a child,
    ...
    for person in people:
        ...
        if mother is None and father is None:
            # No parental info: unconditional probability
            p_gene = PROBS["gene"][g]
        else:
            # With parents: compute based on inheritance and mutation
            gm = gene_count(mother)
            gf = gene_count(father)
            ...
            if g == 2:
                ...
                p_gene = pm * (1 - pf) + (1 - pm) * pf
            else:  # g == 0
                p_gene = (1 - pm) * (1 - pf)

        # Trait probability conditional on gene count
